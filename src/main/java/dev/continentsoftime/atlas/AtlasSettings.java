 ...
 * @param eraAccurate      whether structures and cave biomes that did not exist in an era's version stay off its
 *                         continent (see {@code atlas.timeline}).
 */
public record AtlasSettings(List<Identifier> eras, int maxContinentSize, int oceanWidth, boolean oceans, boolean eraAccurate) {
	public static final Codec<AtlasSettings> CODEC = RecordCodecBuilder.create(i -> i.group(
		...
		Codec.BOOL.optionalFieldOf("era_accurate").forGetter(s -> Optional.of(s.eraAccurate))
	...
	private static AtlasSettings fromJson(Optional<List<Identifier>> eras, Optional<Integer> maxContinentSize, Optional<Integer> oceanWidth,
	                                      Optional<Boolean> oceans, Optional<Boolean> eraAccurate) {
		...
		return new AtlasSettings(
			...
			eraAccurate.orElse(config.eraAccurate())
		);
	}
